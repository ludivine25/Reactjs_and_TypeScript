...
export type Genre = "Aventure" | "Drame" | "Horreur" | "SF" | "Thriller"
...
export function titreDuFilm(liste: Film[], id: number): string | undefined {
  return trouverParId(liste, id)?.titre; // Le point d'interrogation permet de ne pas lever d'erreur si le film n'existe pas
}
...
export function chargerFavoris(): unknown {
  const brut = localStorage.getItem("favoris");
  if (brut === null) return null;  // ou undefined
  return JSON.parse(brut);
}

export function enregistrerFavoris(favoris: unknown) {
...
export function mettreAJour(film: Film, modifications: Partial<Omit<Film, 'id'>>) {
...
// Omit<Film, 'id'> = Film complet SANS la propriété 'id'
// Utile car à la création, l'id n'existe pas encore (le serveur le fournira)
